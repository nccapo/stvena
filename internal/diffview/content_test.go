...
func TestContentSelectsIndexWorktreeAndDeletedVersions(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		content := LoadContent(ws, File{Path: name, Scope: tc.scope, Status: "M"})
	...
	content := LoadContent(ws, File{Path: name, Scope: Unstaged, Status: "D"})
	if content.Err != nil || !content.Old || content.Lines[1] != "staged" {
		t.Fatalf("wrong unstaged deletion: %+v", content)
	}
	runGit(t, root, "add", ".")
	content = LoadContent(ws, File{Path: name, Scope: Staged, Status: "D"})
...
func TestFullUntrackedFileBeyondPatchPreview(t *testing.T) {
	...
	content := LoadContent(ws, File{Path: "large.txt", Scope: Untracked})
	...
	if !LoadContent(ws, File{Path: "binary", Scope: Untracked}).Binary {
		t.Fatal("binary not detected")
	}
	writeFile(t, root, "empty", "")
	content = LoadContent(ws, File{Path: "empty", Scope: Untracked})
	if content.Err != nil || len(content.Lines) != 0 {
		t.Fatal("empty file misread")
	}
	if LoadContent(ws, File{Path: "missing", Scope: Untracked}).Err == nil {
...
func TestFullFileSymlinkAndLimit(t *testing.T) {
	...
	content := LoadContent(ws, File{Path: "link", Scope: Untracked})
	...
	if LoadContent(ws, File{Path: "too-big", Scope: Untracked}).Err == nil {
